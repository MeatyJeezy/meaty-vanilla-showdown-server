...
import { RoomBattlePlayer } from "../server/room-battle";
import { RoomGame, RoomGamePlayer, SimpleRoomGame } from "../server/room-game";
...
export const Rulesets: {[k: string]: FormatData} = {
	...
	dynamaxormega: {
		...
		onTeamPreview() {
			//const msg = 'uhtml|otsrequest|<button name="send" value="/wantdynamax" class="button" style="margin-right: 10px;"><strong>Dynamax</strong></button><button name="send" value="/wantmegazmove" class="button" style="margin-top: 10px"><strong>Mega + Z-Moves</strong></button>';
			for (const side of this.sides) { 
				//this.addSplit(side.id, [msg]);
				for (const pokemon of side.pokemon) {
					if (pokemon.getItem().megaStone != undefined || pokemon.getItem().zMove == true) {
						// pokemon.canMegaEvo = null;
						// side.zMoveUsed = true;
						side.dynamaxUsed = true;
					}
